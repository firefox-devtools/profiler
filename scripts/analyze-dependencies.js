  ...
  function walkDir(dir) {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        if (!file.includes('test') && file !== 'libdef' && file !== 'node_modules') {
          walkDir(fullPath);
        }
      } else if (stat.isFile()) {
        ...
        if (relativePath.includes('/test/') || relativePath.includes('/libdef/')) {
          continue;
        }
  ...
  try {
    ...
    for (const regex of patterns) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (importPath.startsWith('./') || importPath.startsWith('../') || importPath.startsWith('firefox-profiler/')) {
...
function buildDependencyGraph(allFiles) {
  ...
  for (const file of allFiles) {
    ...
    const resolvedImports = [...new Set(imports
...
function calculateTransitiveDependencies(file, graph, visited = new Set()) {
  ...
  for (const dep of fileInfo.directDependencies) {
    ...
    const transitiveOfDep = calculateTransitiveDependencies(dep, graph, new Set(visited));
...
function analyzeFiles() {
  console.log('🔍 Analyzing JavaScript and TypeScript files and their dependencies...\n');
  ...
  fileAnalysis.sort((a, b) => {
    ...
    if (a.transitiveUnconvertedDependencies !== b.transitiveUnconvertedDependencies) {
      return a.transitiveUnconvertedDependencies - b.transitiveUnconvertedDependencies;
    }
  ...
  console.log('🎯 Files ordered by ease of fixing (transitive unconverted deps : direct unconverted deps : lines : file):');
  console.log('   Note: "Direct dependencies" = files imported directly by this file');
  console.log('   Note: "Transitive dependencies" = all unconverted files this file depends on (directly or indirectly)\n');
  ...
  for (const analysis of fileAnalysis) {
    ...
    console.log(`${emoji} ${analysis.transitiveUnconvertedDependencies} trans, ${analysis.directUnconvertedDependencies} direct, ${analysis.lineCount} lines: ${fileType} ${relativePath}`);
    
    // Show problematic dependencies for files that aren't ready
    if (analysis.transitiveUnconvertedDependencies > 0 && analysis.transitiveUnconvertedDependencies <= 10) {
  ...
  console.log('   1. Convert 🟢 files first (no transitive unconverted dependencies)');
  ...
  console.log('\n💡 Tips:');
  console.log('   • Convert smaller files within each category first for easier testing');
  console.log('   • [TS] files may still need fixes to pass strict type checking');
  console.log('   • Focus on direct dependencies to maximize impact on other files');
  
  // Summary statistics
  const readyFiles = fileAnalysis.filter(f => f.transitiveUnconvertedDependencies === 0);
  ...
  console.log(`   • ${tsNeedingFixes.length} TS files may need strict type checking fixes`);
  console.log(`   • ${fileAnalysis.length - readyFiles.length} files blocked by dependencies`);
}
