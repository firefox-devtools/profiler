...
describe('marker info with several handles', () => {
  ...
  it('rejects specs that expand to nothing', async () => {
    // `specs` is non-empty here, so the emptiness only shows up after
    // expansion; without that check the command prints just the context
    // header and exits 0.
    for (const args of [
      ['marker', 'info', ''],
      ['marker', 'info', ','],
    ]) {
      const result = await cliFail(ctx, args);
  ...
  it('tells the user that marker stack does not take ranges', async () => {
    ...
    // "Unknown marker m-1..m-2" would read like a bad handle instead of
    // unsupported syntax, so the range must be rejected before lookup.
  ...
  it('accepts marker stack spellings that mean a single handle', async () => {
    // These expand to exactly m-2, so the expansion is what gets looked up;
    // passing the raw spec through would fail as an unknown marker.
    for (const spec of ['m-2..m-2', 'm-2,', ',m-2']) {
      const result = await cli(ctx, ['marker', 'stack', spec]);

      expect({ spec, exitCode: result.exitCode }).toEqual({
        spec,
        exitCode: 0,
      });
      expect(result.stdout).toContain('m-2');
  ...
  it('rejects a marker stack spec that expands to nothing', async () => {
    const result = await cliFail(ctx, ['marker', 'stack', ',']);

    expect(result.exitCode).toBe(1);
    expect(result.stdout + result.stderr).toContain(
      'marker stack requires a marker handle'
    );
  });
});
