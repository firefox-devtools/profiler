...
describe('formatThreadListResult', function () {
  ...
  it('uses the singular when --limit hid a single row', function () {
    const output = formatThreadListResult(
      listOf([makeThread()], { totalThreadCount: 2, hiddenByLimit: 1 })
    );

    expect(output).toContain('+ 1 more thread (use --limit 0 to see all)');
  });
