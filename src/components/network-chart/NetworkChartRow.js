...
const NetworkChartRowBar = (props: NetworkChartRowBarProps) => {
  ...
  const partialDuration =
    markerMeta.requestQueue + markerMeta.request + markerMeta.response;
...
class NetworkChartRow extends React.PureComponent<NetworkChartRowProps, State> {
  ...
  _createMarkerMetaData(
    ...
    if (
      ...
      request = networkPayload.responseStart - networkPayload.requestStart; // create object to throw into tooltip
    }
    ...
    if (
      ...
      response = networkPayload.responseEnd - networkPayload.responseStart;
    }
