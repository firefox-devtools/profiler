 ...
 */
const BARE_KEYMAP: { [key: string]: NavigationKey } = {
  KeyQ: 'zoomIn',
  KeyY: 'zoomIn',
  KeyE: 'zoomOut',
  KeyU: 'zoomOut',
 ...
 */
const CTRL_KEYMAP: { [key: string]: NavigationKey } = {
  ArrowUp: 'zoomIn',
  ArrowDown: 'zoomOut',
};
...
export type Viewport = {
  ...
  readonly moveViewport: (dx: CssPixels, dy: CssPixels) => void;
...
> {
  ...
  _keysDown: Set<NavigationKey> = new Set<NavigationKey>();
  ...
  zoomRangeSelection = (
    ...
    const { disableHorizontalMovement, maximumZoom } = this.props.viewportProps;
    if (disableHorizontalMovement) {
      return;
    }
  ...
  _keyDownListener = (
    event: { nativeEvent: KeyboardEvent } & React.KeyboardEvent<HTMLElement>
  ) => {
    let navigationKey;
