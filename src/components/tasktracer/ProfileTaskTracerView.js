  ...
  render() {
    ...
    for (const taskIndex of tasks) {
      ...
      let track = tracks.find(
        track => track.fullUpTo <= taskTable.dispatchTime[taskIndex]
      );
      if (track === undefined) {
        track = {
          tasks: [],
          ...
          return (
            <ol className="taskTracerThreadViewTrack" key={trackIndex}>
              {track.tasks.map((taskIndex, i) => {
                ...
                const pos =
                  (beginTime - rangeStart) / (rangeEnd - rangeStart) * width;

                const execWidth =
                  (endTime - beginTime) / (rangeEnd - rangeStart) * width;
                const beginDelayWidth =
                  (beginTime - dispatchTime) / (rangeEnd - rangeStart) * width;

                const addressIndex = taskTable.address[taskIndex];
                const classNameStringIndex =
                  taskTable.ipdlMsg[taskIndex] !== undefined
                    ? taskTable.ipdlMsg[taskIndex]
                    : addressTable.className[addressIndex];
                const labelStringIndices = taskTable.label[taskIndex];
                let title =
                  classNameStringIndex !== undefined
                    ? stringTable.getString(classNameStringIndex)
                    : 'undefined';
                if (labelStringIndices !== undefined) {
                  title +=
                    '\n' +
                    labelStringIndices
                      .map(index => ` - ${stringTable.getString(index)}`)
                      .join('\n');
                }
                return (
                  <li
  ...
  render() {
    ...
    return (
      <li className="taskTracerThreadView">
        <h1 className="taskTracerThreadViewThreadName" title={name}>
          {name}
        </h1>
        <ThreadTaskTracerTracks
          tasks={tasks}
