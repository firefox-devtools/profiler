...
describe('function-list', function () {
  ...
  describe('truncateFunctionName', function () {
    ...
    it('preserves C++ operator names with unmatched angle/paren brackets', function () {
      expect(truncateFunctionName('foo::operator>>(int)', 120)).toBe(
        'foo::operator>>(int)'
      );
      expect(truncateFunctionName('foo::operator<<(int)', 120)).toBe(
        'foo::operator<<(int)'
      );
      expect(truncateFunctionName('foo::operator->()', 120)).toBe(
        'foo::operator->()'
      );
    });
