...
import {
  collectSliceTree,
  printSliceTree,
} from 'firefox-profiler/profile-query/cpu-activity';
...
describe('profile-query cpu activity', function () {
  ...
  it('prints a fallback message when there are no slices', function () {
    const tsManager = new TimestampManager({ start: 0, end: 10 });

    expect(printSliceTree({ slices: [], time: [] }, tsManager)).toEqual([
