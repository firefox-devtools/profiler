...
describe('sanitizePII', function () {
  ...
  const extensionMarkerSchemaByName: MarkerSchemaByName = Object.fromEntries(
    extensionMarkerSchemas.map((schema) => [schema.name, schema])
  );
  ...
  it('should sanitize URLs after upgrading an extension text marker', function () {
    ...
    if (!marker || marker.type !== 'ExtensionSuspend') {
      throw new Error('Expected an ExtensionSuspend marker');
    }
  ...
  it('should sanitize PII inside structured extension markers', function () {
    ...
    for (const marker of [markers.data[0], markers.data[1]]) {
      if (
        !marker ||
        (marker.type !== 'ExtensionParent' && marker.type !== 'ExtensionChild')
      ) {
