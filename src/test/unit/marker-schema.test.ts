...
describe('marker schema labels', function () {
  type LabelOptions = {
    schemaFields: MarkerSchema['fields'];
    label: string;
    payload: any;
  };
  ...
  describe('parseErrors', function () {
    function testParseError(label: string) {
      ...
      expect(console.error).toHaveBeenCalledTimes(1);
      // @ts-expect-error - mock property doesn't exist
...
describe('marker schema formatting', function () {
  ...
  it('can apply a variety of formats', function () {
    const entries: [MarkerFormatType, unknown][] = [
      ...
      ['hexadecimal', 0],
      ['hexadecimal', 255],
      ['hexadecimal', 4294967295], // largest 32-bit flag value
      ['percentage', 0],
      ['percentage', 0.1],
