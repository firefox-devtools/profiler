...
describe('getLineTimings for getStackLineInfo', function () {
  ...
  it("attributes the func's original line when the frame is not source-mapped but the func is", function () {
    ...
    // source, so line attribution must also come from the func's sourceMapInfo
    ...
    // For both funcs, add a sourceMapInfo entry that points to the original
    // source at a known line. Leave frameTable.sourceMapInfo as null on every
    // frame — that is the case under test.
    const funcAIndex = frameTable.func[stackTable.frame[0]];
    const funcBIndex = frameTable.func[stackTable.frame[stackTable.length - 1]];
    const funcALine = 100;
    const funcBLine = 200;
    const smi = thread.sourceMapInfo;
    smi.originalSource.push(originalSourceIndex, originalSourceIndex);
    smi.originalLine.push(funcALine, funcBLine);
    smi.originalColumn.push(1, 1);
    const funcAInfoIdx = smi.length;
    const funcBInfoIdx = smi.length + 1;
    smi.length += 2;
    funcTable.sourceMapInfo[funcAIndex] = funcAInfoIdx;
    funcTable.sourceMapInfo[funcBIndex] = funcBInfoIdx;
...
describe('getTotalLineTimingsForCallNode', function () {
  function getTimings(
    ...
    const { stackTable, frameTable, funcTable, samples, sourceMapInfo } =
      thread;
