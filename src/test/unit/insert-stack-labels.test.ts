...
describe('insertStackLabels', function () {
  it('inserts a label before the first frame matching a bucket', function () {
  ...
  it('does not repeat the label for consecutive frames in the same bucket', function () {
    const { profile } = getProfileFromTextSamples(`
      A
      gfx::Composite
  ...
  it('does not re-insert the label when re-entering a bucket after an unmatched frame', function () {
    // A non-JS, non-bucket frame (helper) propagates the inherited bucket context,
    // so gfx::DrawQuad is still considered inside the Graphics bucket and no new
    // label is inserted.
    const { profile } = getProfileFromTextSamples(`
      gfx::Composite
      helper
  ...
  it('inserts the right label when multiple buckets are configured', function () {
    const { profile } = getProfileFromTextSamples(`
      A               A
  ...
  it('inserts a new label when switching from one bucket to another', function () {
    const { profile } = getProfileFromTextSamples(`
      layout::Reflow
      gfx::Composite
    `);
