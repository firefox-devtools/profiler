...
describe('origin annotation', function () {
  ...
  function addResource(
    ...
    const resourceIndex = profile.shared.resourceTable.length;
    const funcIndex = funcNames.indexOf(funcName);
    profile.shared.funcTable.resource[funcIndex] = resourceIndex;
    profile.shared.funcTable.source[funcIndex] = location
      ? addSourceToTable(
          profile.shared.sources,
          stringTable.indexForString(location)
        )
      : null;
    profile.shared.resourceTable.lib.push(-1);
    profile.shared.resourceTable.name.push(stringTable.indexForString(name));
    profile.shared.resourceTable.host.push(
      host ? stringTable.indexForString(host) : null
    );
    profile.shared.resourceTable.length++;
  }
