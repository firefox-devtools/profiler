  ...
  describe('match function names', function () {
    it('works on the sampled data', function () {
...

      {
        ...
        const fooLine = 3;
        const fooColumn = 5;
        profile.shared.funcTable.lineNumber[foo] = fooLine;
        profile.shared.funcTable.columnNumber[foo] = fooColumn;
        const fooUrlIndex = stringTable.indexForString('https://mozilla.org');
        profile.shared.funcTable.source[foo] = addSourceToTable(
        ...
        const barLine = 7;
        const barColumn = 11;
        profile.shared.funcTable.lineNumber[bar] = barLine;
        profile.shared.funcTable.columnNumber[bar] = barColumn;
        const barUrlIndex = stringTable.indexForString('https://mozilla.org');
        profile.shared.funcTable.source[bar] = addSourceToTable(
          profile.shared.sources,
          barUrlIndex
        );
        ...
        profile.shared.funcTable.lineNumber[baz] = barLine;
        profile.shared.funcTable.columnNumber[baz] = barColumn;
        const bazUrlIndex = stringTable.indexForString('https://mozilla.org');
        profile.shared.funcTable.source[baz] = addSourceToTable(
          profile.shared.sources,
          bazUrlIndex
        );
