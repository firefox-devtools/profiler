...
describe('TrackCustomMarker with committed range scaling', function () {
  ...
  it('uses the committed range scaling selector correctly', function () {
    ...
    expect(fullRangeSamples.minNumber).toBe(1);
    expect(fullRangeSamples.maxNumber).toBe(200); // 100 * 2 from second line
    ...
    expect(committedRangeSamples.minNumber).toBe(2);
    expect(committedRangeSamples.maxNumber).toBe(6);
  ...
  it('handles edge case where committed range has identical values', function () {
    ...
    const state = getState();
    const committedRangeSamples =
      markerTrackSelectors.getCommittedRangeCollectedCustomMarkerSamples(state);
    expect(committedRangeSamples.minNumber).toBe(5);
    expect(committedRangeSamples.maxNumber).toBe(10); // 5 * 2 from second line
  });
});
