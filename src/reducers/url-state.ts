  ...
  switch (action.type) {
    ...
    case 'SANITIZED_PROFILE_PUBLISHED': {
      ...
      for (const [threadsKey, transformStack] of objectEntries(state)) {
        ...
        newTransforms[newThreadsKey] = translateTransformStack(transformStack, translationMaps);
      }
...
const sourceView: Reducer<SourceViewState> = (
  ...
  switch (action.type) {
    ...
    case 'SANITIZED_PROFILE_PUBLISHED': {
      ...
      const newSourceIndexPlusOne = action.translationMaps.oldSourceToNewSourcePlusOne[state.sourceIndex];
      return {
        ...state,
        sourceIndex: newSourceIndexPlusOne !== 0 ? newSourceIndexPlusOne - 1 : null,
      }
    }
