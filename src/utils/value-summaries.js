...
function bufferIsValid(valuesBuffer) {
  const reader = new BufferReader(valuesBuffer, 0);
  const version = reader.readUint32();
  return version === EXPECTED_VALUE_SUMMARIES_VERSION;
}
...
function getArgumentSummaries(valuesBuffer, shapes, valuesBufferIndex) {
  ...
  if (!bufferIsValid(valuesBuffer)) {
    throw new Error(
      'Unexpected tracer arguments buffer format. Either Firefox or the profiler likely need to be updated.'
    );
  }
