...
export function extractFuncsAndResourcesFromFrameLocations(
  ...
  const extractionInfo: ExtractionInfo = {
    ...
    globalDataCollector,
    processSourceTable: processSourceTable,
  };
...
function _extractJsFunction(
  ...
  if (sourceIndex !== undefined) {
    ...
    if (processSourceTable && geckoSourceIdx < processSourceTable.data.length) {
      const uuidIndex = processSourceTable.schema.uuid;
      const uuid = processSourceTable.data[geckoSourceIdx][uuidIndex];
...
/**
 * Process sources table from all processes  and populate the global data
 * collector with UUID-to-index mappings.
 */
function _processSourcesTable(
  geckoProfile: GeckoProfile,
  globalDataCollector: GlobalDataCollector
): void {
...
export function processGeckoProfile(geckoProfile: GeckoProfile): Profile {
  ...
  _processSourcesTable(geckoProfile, globalDataCollector);
