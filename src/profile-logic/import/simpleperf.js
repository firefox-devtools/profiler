...
import type { CategoryList, FrameTable, FuncTable, IndexIntoCategoryList, IndexIntoFrameTable, IndexIntoFuncTable, IndexIntoResourceTable, IndexIntoStackTable, ProfileMeta, ResourceTable, SerializableSamplesTable, SerializableProfile, SerializableThread, StackTable } from 'firefox-profiler/types/profile';
...
class FirefoxThread {
    ...
    addSample(sample: report.ISample, fileMap: Map<number, report.IFile>): void {
        ...
        for (const frame of sample.callchain.reverse()) {
            ...
            const methodName = frame.symbolId >= 0 ? file.symbol[frame.symbolId] : `${file.path.split(/[\\/]/).pop()}+0x${frame.vaddrInFile?.toString(16)}`;
...
export class SimpleperfReportConverter {
    ...
    static verifyMagic(traceBuffer: ArrayBuffer): boolean {
        return new TextDecoder('utf8').decode(traceBuffer.slice(0, this.magic.length)) === this.magic
    }
    ...
    readUint16LE() {
        const value = this.bufferView.getUint16(this.bufferOffset, true /* littleEndian */);
        this.bufferOffset += 2;
        return value;
    }

    readUint32LE() {
        const value = this.bufferView.getUint32(this.bufferOffset, true /* littleEndian */);
        this.bufferOffset += 4;
        return value;
    }
...
export function convertSimpleperfTraceProfile(traceBuffer: ArrayBuffer): SerializableProfile {
