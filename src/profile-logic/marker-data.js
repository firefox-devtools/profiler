...
export function computeFlowMarkers(
  ...
  for (
    ...
    for (const { key, isTerminating } of flowSchema.flowFields) {
      const flowIDStringIndex = markerData[key];
      if (flowIDStringIndex !== undefined && flowIDStringIndex !== null) {
        const flowID = stringTable.getString(flowIDStringIndex);
...
export function computeProfileFlowInfo(
  ...
  const flowMarkersPerThread: FlowMarker[][] = fullMarkerListPerThread.map(
    (fullMarkerList, threadIndex) => {
      const { stringTable } = threads[threadIndex];
      return computeFlowMarkers(fullMarkerList, stringTable, flowSchemasByName);
    }
  );
  ...
  const flowMarkerFlowsPerThread = flowMarkersPerThread.map(() => []);

  const flowTable = [];
  const currentActiveFlows = new Map();
...
export function computeMarkerFlows(
  ...
  profileFlowInfo: ProfileFlowInfo,
  stringTablePerThread: UniqueStringArray[],
  fullMarkerListPerThread: Marker[][]
): IndexIntoFlowTable[] | null {
  ...
  const stringTable = stringTablePerThread[threadIndex];

  const flowIndexes = [];
  for (const { key } of flowSchema.flowFields) {
    const fieldValue = markerData[key];
...
export function computeFlowTiming(
  ...
  const rawRows: Array<[FlowTimingRowType, IndexIntoFlowTable]> = [
    ...incomingContextFlows.map((flowIndex) => ['INCOMING_CONTEXT', flowIndex]),
    ...activeFlows.map((flowIndex) => ['ACTIVE', flowIndex]),
    ...directlyConnectedFlows.map((flowIndex) => [
      'DIRECTLY_CONNECTED',
      flowIndex,
    ]),
    ...outgoingContextFlows.map((flowIndex) => ['OUTGOING_CONTEXT', flowIndex]),
  ];
