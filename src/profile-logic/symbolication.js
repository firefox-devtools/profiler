...
function _removeFramesFromStackTable(
  ...
  const prefixMap: Map<IndexIntoStackTable, IndexIntoStackTable | null> =
    new Map();
  for (let stack = 0; stack < stackTable.length; stack++) {
    const frame = stackTable.frame[stack];
    const prefix = stackTable.prefix[stack];
    const newPrefix = prefix === null ? null : prefixMap.get(prefix) ?? null;
    ...
    prefixMap.set(stack, newStackIndex);
  }
