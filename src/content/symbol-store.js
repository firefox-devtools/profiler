...
export class SymbolStore {
  ...
  _ensureLibraryIsInDB(lib) {
    ...
    return this._db.getLibKey(debugName, breakpadId).catch(() => {
      ...
      const symbolTablePromise = this._symbolProvider.requestSymbolTable(lib.debugName, lib.breakpadId);
      const importPromise = symbolTablePromise.then(symbolTable => {
        this._importingLibs.delete(libid);
        return this._db.importLibrary(debugName, breakpadId, symbolTable);
      }, error => {
        this._importingLibs.delete(libid);
   ...
   * @param  {object} lib A library object with the properties `debugName` and `breakpadId`.
