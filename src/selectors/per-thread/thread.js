...
import { base64StringToBytes, base64StringToBytesTEST } from '../../utils/base64';
...
export function getBasicThreadSelectorsPerThread(
  ...
  const getTracedValuesBuffer: Selector<ArrayBuffer | null> = createSelector(
    (state) => getRawThread(state).tracedValuesBuffer,
    (tracedValuesBuffer) =>  tracedValuesBuffer ? base64StringToBytes(tracedValuesBuffer) : null
  );
