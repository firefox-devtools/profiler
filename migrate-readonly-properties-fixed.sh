...
# Converts "+prop: Type" to "readonly prop: Type" - with careful regex to avoid false positives

set -e

echo "🔍 Finding files with readonly properties (refined search)..."

# Find all JavaScript files with readonly properties - looking for the specific pattern
...
# Count total occurrences with the specific pattern
...
for file in $files_with_readonly; do
    ...
    # This regex looks for exactly 2 spaces followed by + and property name
